...
public class PlayingCard implements Comparable<PlayingCard> {
    ...
    public int compareTo(PlayingCard other) {
        int suitDiff = this.suit.compareTo(other.suit);
        if(suitDiff != 0) {
            return suitDiff;
        }
        return this.getRank().getRankNumber() - other.getRank().getRankNumber();
    }
    public static void main(String[] args) {
        
        java.util.List<PlayingCard> cards = new java.util.LinkedList<>();
        cards.add(new PlayingCard(Suit.DIAMONDS, Rank.ACE));
